            ...
            <div className="w-2/12 border-l-2 border-indigo-700">
                ...
                {isOptionOpen === true &&
                    <div className="h-32 w-14 mt-1 border border-indigo-700 rounded-sm bg-white overflow-hidden overflow-y-auto absolute">
                        <div className="text-center py-2 border-b-2 border-slate-300" onClick={() => {changeRate((prev) => {prev[name] = 1; return prev}); update((prev) => !prev)}}>1</div>
                        <div className="text-center py-2 border-b-2 border-slate-300" onClick={() => {changeRate((prev) => {prev[name] = 2; return prev}); update((prev) => !prev)}}>2</div>
                        <div className="text-center py-2 border-b-2 border-slate-300" onClick={() => {changeRate((prev) => {prev[name] = 3; return prev}); update((prev) => !prev)}}>3</div>
                        <div className="text-center py-2 border-b-2 border-slate-300" onClick={() => {changeRate((prev) => {prev[name] = 4; return prev}); update((prev) => !prev)}}>4</div>
                        <div className="text-center py-2 border-b-2 border-slate-300" onClick={() => {changeRate((prev) => {prev[name] = 5; return prev}); update((prev) => !prev)}}>5</div>
                    </div>
                }
            </div>
        </div>
